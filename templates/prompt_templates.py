def get_learning_prompt(topic, target_word_count, language='fr'):
    ...
    prompts = {
        ...
        """,
        'ar': f"""
        قم بإنشاء مقتطف تعليمي جذاب حول الموضوع: {topic}.
        
        الإرشادات:
        1. ابدأ بعنوان جذاب يبدأ بـ '# '
        2. اكتب محتوى يبلغ حوالي {target_word_count} كلمة (لمدة 5 دقائق صوتية تقريبًا)
        3. استخدم أسلوبًا محادثًا، كما لو كنت تتحدث إلى صديق فضولي
        4. قم بترتيب المحتوى بمقدمة ونقاط رئيسية وخاتمة
        5. قم بتضمين 2-3 حقائق مدهشة أو غير معروفة
        6. تجنب المصطلحات التقنية المفرطة، ولكن لا تبسط كثيرًا
        7. اختتم بتأمل أو سؤال يثير الفضول
        
        يجب أن يكون المحتوى غنيًا بالمعلومات، ممتعًا، وسهل المتابعة عند الاستماع.
        اكتب كل المحتوى بالعربية.
        """
    }
...
def get_recommendation_prompt(previous_topics, count, language='fr'):
    ...
    prompts = {
        ...
        """,
        'ar': f"""
        بناءً على المواضيع التالية التي شاهدها المستخدم:
        {topics_str}
        
        اقترح {count} موضوع(ات) جديد(ة) قد يثير(ون) اهتمامه.
        لكل موضوع، قدم عنوان الموضوع فقط، دون شرح.
        
        تأكد من أن المواضيع:
        1. مرتبطة بالاهتمامات المعبر عنها، ولكنها مختلفة بما يكفي لتوسيع الأفق
        2. محددة وملموسة بدلاً من الغامضة
        3. مثيرة للاهتمام ومن المحتمل أن تثير الفضول
        
        قم بتهيئة كل موضوع كعنصر قائمة مع واصلة.
        أجب بالعربية.
        """
    }
